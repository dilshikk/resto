...
def _make_checklist_out(cl, branch_name, total, completed, skipped) -> ChecklistOut:
    return ChecklistOut(
        id=cl.id,
...
async def _build_out(cl: Checklist, db: AsyncSession) -> ChecklistOut:
    ...
    items = (await db.execute(select(ChecklistItem).where(ChecklistItem.checklist_id == cl.id))).scalars().all()
    return _make_checklist_out(
        cl, branch.name if branch else "—",
        len(items), sum(1 for i in items if i.is_completed), sum(1 for i in items if i.is_skipped),
    )


async def _build_items_batch(items: list[ChecklistItem], db: AsyncSession) -> list[ChecklistItemOut]:
    if not items:
        return []
    ...
    if completer_ids:
        completers = {
            e.id: e.full_name
            for e in (await db.execute(select(Employee).where(Employee.id.in_(completer_ids)))).scalars().all()
        }

    item_ids = [i.id for i in items]
    all_photos = (
        await db.execute(
            select(Photo).where(Photo.checklist_item_id.in_(item_ids)).order_by(Photo.checklist_item_id, Photo.created_at)
        )
    ).scalars().all()
    ...
    if uploader_ids:
        uploaders = {
            e.id: e.full_name
            for e in (await db.execute(select(Employee).where(Employee.id.in_(uploader_ids)))).scalars().all()
        }
    ...
    if standard_codes:
        standards = {
            s.code: s.title
            for s in (await db.execute(select(Standard).where(Standard.code.in_(standard_codes)))).scalars().all()
        }
    ...
    for item in items:
        ...
        result.append(
            ChecklistItemOut(
                id=item.id, checklist_id=item.checklist_id, title=item.title,
                description=item.description, is_required=item.is_required,
                sort_order=item.sort_order, is_completed=item.is_completed,
                ...
                completed_at=item.completed_at, note=item.note, photos=item_photos,
...
async def _assert_previous_required_done(item: ChecklistItem, items: list[ChecklistItem]) -> None:
    for other in items:
        ...
        if other.is_required and _is_item_pending(other):
            raise HTTPException(
                status_code=400,
                detail=f"Нельзя пропустить шаг: сначала выполните обязательный пункт «{other.title}» (шаг {other.sort_order + 1})",
            )


async def _assert_completion_requirements(item: ChecklistItem, note: str | None, db: AsyncSession) -> None:
    """Enforce photo and comment requirements before marking item as completed."""
    ...
    if item.requires_photo:
        photo_count = len(
            (await db.execute(select(Photo).where(Photo.checklist_item_id == item.id))).scalars().all()
        )
        if photo_count == 0:
            raise HTTPException(
                status_code=400,
...
async def list_checklists(
    ...
    all_items = (await db.execute(select(ChecklistItem).where(ChecklistItem.checklist_id.in_(cl_ids)))).scalars().all()
    items_by_cl: dict[int, list] = {}
    for it in all_items:
        items_by_cl.setdefault(it.checklist_id, []).append(it)

    return [
        _make_checklist_out(
            cl, branches_map.get(cl.branch_id, "—"),
            len(items_by_cl.get(cl.id, [])),
            sum(1 for i in items_by_cl.get(cl.id, []) if i.is_completed),
            sum(1 for i in items_by_cl.get(cl.id, []) if i.is_skipped),
        )
        for cl in visible
    ]
...
async def create_checklist(
    ...
    now = datetime.now(timezone.utc)
    due_at = (now + timedelta(minutes=tpl.deadline_offset_minutes)) if tpl.deadline_offset_minutes else None

    cl = Checklist(
        template_id=tpl.id, template_name=tpl.name, branch_id=data.branch_id,
        shift=data.shift, date=data.date, status="open", started_at=now, due_at=due_at,
    ...
    for ti in tpl_items:
        db.add(ChecklistItem(
            checklist_id=cl.id, title=ti.title, description=ti.description,
            sort_order=ti.sort_order, is_required=ti.is_required,
    ...
    await log_action(
        db, actor_id=current.id, action="checklist.created",
        entity_type="checklist", entity_id=cl.id,
...
async def get_checklist(
    ...
    return ChecklistDetail(
        id=cl.id, template_id=cl.template_id, template_name=cl.template_name,
        branch_id=cl.branch_id, branch_name=branch.name if branch else "—",
        shift=cl.shift, date=cl.date, status=cl.status,
        total_items=len(ordered),
        completed_items=sum(1 for i in ordered if i.is_completed),
        skipped_items=sum(1 for i in ordered if i.is_skipped),
        created_at=cl.created_at, started_at=cl.started_at, due_at=cl.due_at,
        completed_at=cl.completed_at, deadline_status=_compute_deadline_status(cl),
        items=item_outs,
    )
...
async def get_current_item(
    ...
    if item.standard_code:
        standard = (await db.execute(select(Standard).where(Standard.code == item.standard_code))).scalar_one_or_none()
    ...
    return CurrentItemOut(
        id=item.id, checklist_id=item.checklist_id, title=item.title,
        description=item.description, is_required=item.is_required,
        sort_order=item.sort_order, total_items=len(items), current_position=position,
        standard_code=item.standard_code, standard_title=standard_title,
...
async def toggle_item(
    checklist_id: int, item_id: int, body: ToggleItemRequest,
    ...
    item = (
        await db.execute(select(ChecklistItem).where(ChecklistItem.id == item_id, ChecklistItem.checklist_id == checklist_id))
    ).scalar_one_or_none()
    if not item:
    ...
    await log_action(
        db, actor_id=current.id,
        action="task.completed" if item.is_completed else "task.reopened",
        entity_type="checklist_item", entity_id=item.id,
...
async def skip_item(
    checklist_id: int, item_id: int, body: SkipItemRequest,
    current: Employee = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
    cl = await _get_checklist_or_404(checklist_id, db)
    await _assert_branch_access(cl, current, db)
    ...
    if body.note:
        item.note = body.note
    await log_action(
        db, actor_id=current.id, action="task.skipped",
        entity_type="checklist_item", entity_id=item.id,
        metadata={"checklist_id": checklist_id, "title": item.title, "note": item.note},
    )
    await db.commit()
...
async def complete_checklist(
    ...
    cl.completed_at = now
    await log_action(
        db, actor_id=current.id, action="checklist.completed",
        entity_type="checklist", entity_id=cl.id,
...
async def upload_item_photo(
    ...
    photo = Photo(checklist_item_id=item.id, uploaded_by_employee_id=current.id, url=f"/api/v1/checklists/photos/{filename}")
    db.add(photo)
    await db.flush()
    await log_action(db, actor_id=current.id, action="photo.uploaded", entity_type="checklist_item", entity_id=item.id, metadata={"photo_id": photo.id})
    await db.commit()
    await db.refresh(photo)
    return ChecklistItemPhotoOut(id=photo.id, url=photo.url, uploaded_by_name=current.full_name, created_at=photo.created_at)
...
async def delete_item_photo(
    checklist_id: int, item_id: int, photo_id: int,
    current: Employee = Depends(get_current_user), db: AsyncSession = Depends(get_db),
):
    cl = await _get_checklist_or_404(checklist_id, db)
    await _assert_branch_access(cl, current, db)
    photo = (await db.execute(select(Photo).where(Photo.id == photo_id, Photo.checklist_item_id == item_id))).scalar_one_or_none()
    if not photo:
        raise HTTPException(status_code=404, detail="Фото не найдено")
    ...
    await log_action(db, actor_id=current.id, action="photo.deleted", entity_type="checklist_item", entity_id=item_id, metadata={"photo_id": photo_id})
    await db.commit()
    return {"ok": True}
